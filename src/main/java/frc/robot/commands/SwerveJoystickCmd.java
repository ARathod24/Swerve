...
public class SwerveJoystickCmd extends CommandBase {
    ...
    // Limiters to make driving smoother
    ...
    //applying deadband(rejected small inputs if the motors do not go back to 0 for motor protection)
    ...
    // Making the driving smoother
    xSpeed = xLimiter.calculate(xSpeed)*DriveConstants.kTeleDriveMaxSpeedMetersPerSecond;
    ySpeed = yLimiter.calculate(ySpeed)*DriveConstants.kTeleDriveMaxSpeedMetersPerSecond;
    turningSpeed = turningLimiter.calculate(turningSpeed)*DriveConstants.kTeleDriveMaxAngularSpeedRadiansPerSecond;

    // Constructing chassis speeds
    ...
    if (fieldOrientedFunction.get()){
        ...
        // Converting speeds into module states
        SwerveModuleState[] moduleStates = DriveConstants.kDriveKinematics.toSwerveModuleStates(chassisSpeeds);
        //outputting each module state to the wheels
